...
- 前端整体 UI 设计必须优先考虑美观度与统一性。视觉表现上坚持 Agentum 自身的现代企业工作台语言（如适当的高质感卡片、间距、排版与微动效），弱化单纯对某个组件库（如 Ant Design）的教条依赖，通过本地 CSS / Tailwind 补充以达到最佳体验。
...
- 租户管理承担当前租户内管理职责，支持用户、部门、角色、资源范围、可用能力池分配、需求配置和运行安全策略；能力分配不拆“查看 / 使用 / 执行 / 管理”动作，权限边界按平台级系统管理 / 租户内管理分层设计。
