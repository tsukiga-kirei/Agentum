...
- `apps/api` 是控制面和治理面，负责能力登记、单租户能力配置、权限校验、凭证注入、调用审计和版本发布。
...
- 权限分两重：第一重是 `business`、`tenant_admin`、`system_admin` 三大登录入口；第二重是租户管理员按用户、部门、租户自定义角色配置的模块、页签、动作和资源权限。业务用户可以被授予业务工作台、流程设计、能力资产、运行审计等业务侧模块，但实际可见范围必须来自租户内配置并由后端复核。
- 页面之间必须能互相勾稽：系统管理的单租户能力配置决定租户管理可分配的能力；租户管理的角色和资源授权决定业务工作台可见流程；流程设计引用的资产版本必须能在运行审计中追溯。
...
角色 / 租户切换必须作为认证与上下文能力设计。一个用户可同时拥有业务用户、租户管理员、系统管理员等登录入口角色；租户内自定义角色属于第二重权限，不作为登录入口。切换时前端更新活跃角色、租户、左侧菜单、模块内页签和动作按钮，后端仍要按 token、tenantId、roleIds、departmentIds、人员级授权和资源权限复核每个请求。
...
权限配置必须支持租户自定义角色、部门、人员三个维度：角色给默认能力，部门限制组织范围，人员级授权只用于特殊补充并必须写入审计。租户管理员可以分配租户内菜单、模块内页签、资源、动作和敏感审批策略；系统管理员只负责平台级能力登记测试、单租户能力配置和模型分配，不直接代替租户管理员维护日常业务权限。
...
系统管理员在系统管理的租户抽屉中完成“能力配置 / 模型分配”后，租户管理员再在当前租户内分配给角色、部门、智能体或工作流。

凭证策略、模型密钥和全局 MCP 凭证由系统管理维护；权限模块只判断谁可以申请、审批、调用或查看脱敏状态。

系统管理前端不应长期停留在单页卡片展示。生产结构应在“系统管理”左侧大模块内包含平台概览、租户管理、能力管理和模型管理等页签 / 顶层菜单；租户能力配置与模型分配收敛到单租户抽屉，概览卡片只展示统计和风险入口，具体操作必须进入列表、详情和表单。
...
- 将系统管理为租户启用的 MCP、Skills、提示词模板、交付能力，以及分配给租户的模型供应商继续分配给角色、智能体或工作流。

租户管理不能创建或启停租户，不能读取系统级模型密钥和全局 MCP 凭证明文，不能越过系统管理的单租户能力配置范围。
