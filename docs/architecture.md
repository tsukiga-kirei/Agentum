...
- 租户管理：当前租户内用户、部门、角色、空间、资源范围、能力池分配、需求配置、运行安全策略和审计可见性配置入口。
...
- 权限分两重：第一重是 `business`、`tenant_admin`、`system_admin` 三大登录入口；第二重是租户管理员按用户、部门、租户自定义角色配置的模块、页签、资源范围和可用能力池。能力分配不拆“查看 / 使用 / 执行 / 管理”动作，进入当前主体能力池后，前端可见即能用于资产创建、智能体模板和流程设计。
...
资源范围要覆盖工作流定义、智能体模板、运行记录、变量快照、交付物和审计日志。一个智能体或交付物完成后，只允许分配范围内的用户、部门或角色查看；敏感文件下载和审计日志查看属于运行安全策略，应单独校验和审计。能力资产本身走可用能力池分配，不再拆查看 / 使用 / 执行 / 管理动作。
