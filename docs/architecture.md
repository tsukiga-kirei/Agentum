...
- `capabilities/delivery/` 只保存自定义交付适配器的 Manifest、源码和本地验证材料，发布后再登记为交付能力资产；系统内置交付能力（如邮箱、Word 文档生成、Excel 工作簿生成）由 API / Worker 原生实现，不放入该目录。
...
认证模块还负责首次部署初始化。应用启动时前端先调用公开的 `GET /api/auth/bootstrap-status`；当全库没有任何用户时，前端进入 `/setup`，调用 `POST /api/auth/bootstrap` 创建首个系统管理员。后端必须在事务内再次确认零用户状态，创建 `users` 记录和 `user_role_assignments(role='system_admin', tenant_id=NULL)`，并写入脱敏日志；一旦存在任何用户，该接口必须拒绝，避免绕过系统管理员治理边界。

权限接口必须按公开登录接口、已登录接口、租户内管理接口、系统管理员接口分组；JWT 解析后注入租户上下文，再做角色校验。租户管理员能力在本项目第一阶段落到租户管理入口。
...
- 校验变量引用。
- 发布不可变版本。
- 通过 `workflow_definitions.active_version_id` 区分最近发布版本与当前业务可用版本；业务入口和手工发起读取当前可用版本，历史运行继续绑定原 `workflow_version_id`。
- 名称、简介、协作权限和画布坐标属于非执行信息，不改变设计态发布状态；只有规范化后的节点、连线、变量及节点运行配置变化才需要重新发布。
- `workflow_access_grants` 使用 `principal_type + principal_id` 保存流程协作主体；读取级别支持角色、部门和人员，编辑级别限定人员。权限判断在列表、详情、业务发起和定时任务入口统一按当前有效成员关系解析，详情响应同时返回去重后的最终可见人员及授权来源。
- 输入节点在 `config.inputFields` 中按字段保存 `required`；运行态从发布版本快照读取并由后端复核，历史未声明该属性的字段保持原有必填语义。
...
- 创建 `WorkflowRun`。
- 按版本生成执行计划。
- 手工发起使用工作流当前可用版本；定时任务固定使用其保存的版本 ID，避免版本切换静默改变计划执行协议。
- 执行节点状态机。
- 管理变量快照。
- 处理暂停、恢复、回退、取消、重试。
- 调度并行节点和合并节点。
...
当前运行态已支持标准 MCP SSE 与 Streamable HTTP 接入：系统管理登记端点并测试连接后，平台把 `tools/list` 返回的工具名、描述和 `inputSchema` 保存为管理预览与诊断快照；该快照不作为运行时事实源。运行节点按租户能力池授权复核，并在每个模型推理回合前通过 `initialize` / `notifications/initialized` / `tools/list` 重新发现当前远程工具，再把最新工具契约声明给模型并通过 `tools/call` 执行。这样上一回合因工具删除、重命名或 Schema 漂移产生的可恢复失败，在 observation 回写后可以由下一回合基于最新工具重新规划。工具名、脱敏参数、结果、耗时和失败原因写入 `mcp_call_logs`，MCP 返回的 `isError=true` 必须按失败记录，不能因 HTTP 请求成功而误记为工具成功。外部工具执行或网络故障属于可恢复 observation，允许模型改用其他工具或解释数据缺失；能力未授权、配置无效、当前工具列表为空或显式指定工具已不存在等安全与治理错误仍终止节点。MCP 结构化结果在进入模型观察、运行快照、SSE 和审计前按敏感键递归脱敏；运行页面保存短 `summary` 与完整 `detail`，折叠态展示摘要、展开态展示完整脱敏结果。
...
当前运行态按流程快照中的 `modelProviderId` 复核 `tenant_model_assignments` 并选择租户启用模型，解密模型供应商 API Key 后调用 OpenAI 兼容 / 通义兼容 / Azure OpenAI Chat Completions。`model_providers.reasoning_model` 是推理能力的系统级权威标识；启用深度推理时请求增加 `chat_template_kwargs.enable_thinking`，响应统一解析 `reasoning`、`reasoning_content`，并通过独立 SSE 内容类型和 `reasoning_content` 输出快照留痕。聊天客户端支持 Function Calling 工具声明与 SSE 文本流，并显式请求流式 usage；`model_call_logs` 同时保存供应商原始 usage 与标准化 `input_tokens`、`output_tokens`、`total_tokens`。一次用户消息可能触发多次 ReAct 调用，运行输出按“本轮对话”累计后写入对应 assistant 消息，审计则保留逐次调用和整次运行汇总。Anthropic Messages 协议暂未接入，运行时会返回明确错误而不是生成占位输出。
...
交付能力分为系统内置和自定义适配器。系统内置交付能力由 API / Worker 原生实现，例如邮箱发送、Webhook 调用、Word 文档生成和 Excel 工作簿生成；自定义交付适配器放在 `capabilities/delivery/<delivery-key>/`，通过 Manifest 声明 `runtime`、`entry`、`configSchema`、`inputSchema`、`outputSchema` 和风险等级。OA、PDF、IM、数据库写入等强依赖客户环境和版式规范的通道进入后续个性化集成阶段，不作为阶段一基础治理验收项。高风险交付能力必须走权限校验、审批或二次确认。

当前运行态已支持四类能力化交付：系统内置邮箱发送、Webhook 调用、Word 文档生成和 Excel 工作簿生成。Word 初版由 API 内轻量渲染器把 AI Markdown 转为 `.docx`；Excel 由 API 内轻量渲染器按多 Sheet 模板宽容识别 Markdown 表格、列表、键值块和普通文本并生成 `.xlsx`。文件写入 MinIO/S3 兼容对象存储，并通过 `delivery_records` 记录下载入口；复杂模板、图片、目录、页眉页脚和大文档后续应迁移到 Worker。所有交付动作都会写入 `delivery_records`，并关联租户、运行、节点、流程定义和发布版本；失败时节点和运行进入失败状态。
...
租户管理是当前租户内的管理入口，承担 Agentum 租户管理员能力的落点。
...
- 邮箱、Webhook、Word 文档和 Excel 工作簿基础交付。
