...
- 工作流设计从自由画布收敛为阶段积木编排：先保留草稿、节点、依赖和设计权限边界，再逐步把契约演进为输入内容、智能体协作处理、审查交付三段式。
- 流程设计页按授信报告闭环重做交互心智：输入公司全称后，智能体结合 Skill 与 MCP 获取数据，章节智能体并行生成，组装智能体统一报告，用户最终审查并交付；事实类数据只能重新获取，模型文本可重新生成或追问修改。
...
- 流程设计详情页已从自由画布重做为三阶段积木编排，突出输入内容、智能体协作处理和用户审查交付。
- 运行审计中的数据来源去掉知识库表述。
...
- 流程设计详情页已接入草稿详情与保存 API：已有草稿按阶段积木回显后端节点和依赖，空草稿先载入授信报告起步模板，首次保存后写回真实草稿结构；列表页同步更新积木数与暂停点摘要。
...
| 工作流草稿 API | 创建、查询和保存固定节点与依赖 | 已完成后端第一版、列表页、阶段积木读取 / 保存、最小发布校验、变量声明和正式发布；后续补版本读取与运行态接入，并把契约从图结构演进为阶段 / 步骤结构 |
...
| 4 | 工作流草稿 API | 已完成第一版创建、查询、详情、保存阶段积木、最小发布校验、变量声明和正式发布；后续补版本读取 |
