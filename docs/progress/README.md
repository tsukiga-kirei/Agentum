...
更新时间：2026-05-18（工作流正式发布与不可变版本快照接入）。
...
- 流程设计画布保存时已同步变量声明：变量名称、类型、来源节点、敏感标记和交付标记随草稿图一起读写，变量面板从后端声明恢复，不再只依赖前端临时推导。
...
- 新增工作流发布校验 API：基于已保存草稿图返回结构化校验结果，并新增 `WorkflowPublishValidatorTest` 覆盖连通图、图结构异常、循环和变量依赖错误。
- 新增 Flyway `V202605180001`：建立 `workflow_variable_definitions`，把变量定义从节点字符串数组中拆出；保存草稿图时同步校验变量命名、类型、来源节点和输出一致性，并新增 `WorkflowVariableDeclarationValidatorTest`。
...
| 工作流草稿 API | 创建、查询和保存固定节点与边 | 已完成后端第一版、列表页、画布图读取 / 保存、最小发布校验、变量声明和正式发布；后续补版本读取与运行态接入 |
...
| 4 | 工作流草稿 API | 已完成第一版创建、查询、详情、保存图、最小发布校验、变量声明和正式发布；后续补版本读取 |
| 5 | 变量系统 | 已完成设计态变量声明和上游引用校验；后续补运行态变量快照 |
