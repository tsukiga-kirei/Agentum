...
- 系统管理页已改为模块化工作台：平台概览统计、租户状态、模型供应商与全局能力注册、租户能力授权；数据来自 `/api/system/*`（需 system_admin 登录），并补齐深色模式弹窗表单可读性。
...
| 前端系统管理页 | 从静态卡片改造成平台概览、租户管理、能力管理、模型管理、租户授权、系统审计等顶层菜单 | 已接 `/api/system/*`：模块化工作台 + 表格与注册弹窗，深色模式表单已增强 |
...
| 契约同步 | 去掉知识库字段，补交付和租户授权类型 | 已完成 Schema / TS 类型占位；后续统一补分页参数与分页响应模型 |
