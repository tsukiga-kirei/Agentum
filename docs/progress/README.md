...
- 设计态取消“暂停点”概念，流程按积木顺序推进；旧字段仅为兼容历史列表和表结构保留，保存草稿时后端固定为 `0`。
...
- 工作流草稿保存取消设计态暂停点计数：`WorkflowDraftService` 保存图结构时将历史 `pause_point_count` 兼容字段固定为 `0`，并补测试覆盖顺序积木保存场景。
...
| 工作流草稿 API | 创建、查询和保存固定节点与依赖 | 已完成后端第一版、列表页、阶段积木读取 / 保存、设计目录、最小发布校验、变量声明和正式发布；后续补版本读取、能力引用后端校验与运行态接入，并把契约从图结构演进为阶段 / 步骤结构 |
...
| 4 | 工作流草稿 API | 已完成第一版创建、查询、详情、保存阶段积木、设计目录、最小发布校验、变量声明和正式发布；后续补版本读取和能力引用后端校验 |
