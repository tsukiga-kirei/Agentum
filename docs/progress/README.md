...
当前已完成 **阶段一：框架与基础治理**，后续进入 **阶段二：稳定化、平台增强与知识资产**，客户侧 OA、PDF、IM 等个性化集成与知识资产并行规划、分优先级推进。
...
- 系统内置交付已覆盖邮箱、Webhook、Word 文档和 Excel 工作簿，满足阶段一“可交付、可追溯”的通用闭环。
...
- 交付能力进一步区分系统内置和自定义适配器：邮箱发送、Word 文档生成和 Excel 工作簿生成作为系统内置能力由 API 原生实现，`capabilities/delivery/` 只保存自定义交付适配器 Manifest、源码和测试材料。
...
- 服务器文件日志采用独立 `logfile` profile：控制台输出继续支持 `docker compose logs`，同时按可信 MDC 上下文互斥写入系统日志与租户日志；运行和附件 MQ 消费者恢复完整租户 / 运行链路，本地开发与自动化测试默认不生成文件。
...
- 流程输入字段支持文本、日期和下拉类型，字段配置统一使用右侧独立抽屉，并复用智能体配置的“左侧分类菜单 + 右侧当前配置”布局；文本和下拉的预设值默认允许业务人员修改，日期字段可按年、年月、年月日三级手工选择，当前年、当前日期、当前年月和上个年月也统一收敛到日期字段配置。人工发起时按配置自动预填并控制是否可修改；创建定时任务时可调整文本模板、插入系统变量，也可在动态日期规则与手工日期之间切换，动态内容在每次执行时按计划触发时间和统一业务时区重新计算，解析结果进入输入变量快照。
- 工作流日期变量已统一为明确的新命名：月份和日期分别使用不补零的 `current_month` / `current_day` 与两位数字的 `current_month_padded` / `current_day_padded`，年月使用 `current_year_month` / `previous_year_month`；移除重复的 `year` / `month` / `day`，并通过迁移同步已有草稿、发布快照、提示词和定时任务配置。
- 定时任务创建与编辑抽屉支持实时预览 cron 表达式的下次执行时间；预览由后端复用正式调度解析规则，并返回业务时区，表达式无效时在配置区内直接提示。
...
- 流程版本治理完成第二轮优化（2026-09-02）：新增 `active_version_id` 区分最近发布版本与当前业务可用版本，流程详情可查看并回切历史版本；简介、名称、协作权限和纯画布位置变化不再触发重新发布，相同执行配置重复保存保持原发布状态；手工发起读取当前可用版本，既有运行继续绑定原快照，定时任务按保存的版本 ID 固定执行。
- 流程详情与协作权限体验优化（2026-09-14）：版本记录默认收敛为最近三个并保证当前可用版本可见，可按需展开全部；流程读取 / 使用范围的指定对象扩展为角色、部门和人员，后端按当前组织关系统一解析列表、详情、发起与定时任务访问，并在详情中返回完整可查看人员及授权来源。编辑范围继续只允许直接指定人员。
...
- 智能体集群节点完成执行方式升级：设计态从“并行 / 顺序”收敛为“协同处理 / 接力处理 / 意图分派”，子智能体支持上下调整顺序；接力处理按顺序校验提示词变量引用；意图分派改为抽屉式意图路由配置，每个意图选择目标子智能体并描述命中条件，其他情况可中止、转交指定智能体或返回固定话术。运行态只接受设计时白名单 `intentCode`，只执行命中的子智能体；最终输出变量可配置，输出模板只组合本节点子智能体输出，多个命中按意图清单顺序写入模板。历史 `parallel` / `sequential` 数据通过 `V202607020001` 迁移清洗为新枚举，代码不再保留旧值兼容分支。
- 交付节点完成多交付改造：配置顺序调整为先选择单个 / 多个交付项；单个交付项保留节点级交付方式，多个交付项则把交付方式下沉到每个交付项自身，允许直接交付与 Word / Excel / 邮件等能力交付混排。多交付支持全部执行或按规则触发，规则支持“始终触发 / 命中某个智能体集群子智能体 / 输入节点字段等于固定值 / 单智能体节点有输出内容”。流程设计页把交付项配置改为右侧抽屉，Word 文档交付原有模板、文件名和样式配置继续复用；运行态只执行并展示实际触发的交付项，未触发项不进入产品交付页。历史交付节点通过 `V202607030001` 补齐新配置口径，上一版变量 / 意图规则通过 `V202607030002` 清理，已运行环境通过 `V202607030003` 补齐每个交付项的独立交付方式，并通过 `V202607030004` 更新触发规则语义说明。
- 流程导入模型引用完成收口：来源环境模型供应商 UUID 在当前租户不可用时不再静默展示第一个默认模型，而是清空并要求重新选择；单智能体抽屉改为点击“完成”才写回节点、取消不改配置，与集群智能体一致。发布校验新增模型供应商、租户模型分配、模型名和意图识别模型复核，并继续统一校验 MCP、Skill、智能体 / 提示词模板与交付能力的状态、类型和当前操作者授权。
...
- 智能体集群子项重跑（2026-07-20）：运行页每个已完成子智能体增加「重新执行」；协同处理只替换目标结果，接力处理自动重跑目标及后序依赖，意图分派冻结原命中范围且不重新分类；新增子智能体重跑 API、执行命令范围字段、运行事件与回归测试，底部整组「重新执行」继续保留。
...
- Word 列表与引用排版收敛（2026-07-20）：Markdown 引用不再强制左缩进和斜体，统一按正文样式渲染；系统管理与流程交付节点可分别配置有序、无序列表为继承正文、无缩进或自定义悬挂缩进，逐段个性化首行缩进可覆盖列表全局设置。
- Word 标题与手动空行样式补齐（2026-07-20）：保留首行单独居中的原有设置，并把一至五级标题对齐并入对应级别的标题配置；全局段落区只保留 Word 段距，个性化规则可在目标段落前后插入等价于手动回车的空段落。“继承目标段落 / 继承正文”会把对应字体、字号、行距与段距写入空段落标记，“固定磅值”则使用独立行高并清除空段落段距。
- Excel 工作簿交付落地（2026-07-06）：新增系统内置 `excel_workbook_delivery` 能力、轻量 XLSX 渲染器、运行态路由、MinIO 文件存储内容类型支持、过期清理、流程设计多 Sheet 模板配置、列/行/单元格格式规则和宽容 Markdown/列表/键值/JSON/CSV 解析；新增 [Excel 工作簿交付说明](../excel-workbook-delivery.md)。
- Word、Excel 交付文件保存策略支持“永久 / 按天保存”；永久保存不生成到期时间，历史能力未配置策略时继续沿用原保留天数。
- 运行态与交付体验（2026-07-06）：Excel 交付在线预览、工具调用执行历史展示优化、交付触发变量模板解析。
- 流程设计增强（2026-07-06）：草稿导入导出、指定位置插入积木、智能体 / 交付项复制、保存校验与下拉字段同步优化。
- 工作台与账号（2026-07-06）：定时任务手动触发与权限配置、通知中心、个人资料与密码修改；角色切换与主题切换统一收纳到左下角人物菜单。
- 文档与界面（2026-07-06）：根 README 补充架构图、界面截图与 PR 说明；移除「代码中的实现对应」章节。
...
阶段一完成后，后续开发不再继续扩大基础框架范围，优先围绕「稳定、可验收、可接客户系统」推进；**下一阶段开发主线是知识资产**，把企业文档纳入与能力资产一致的治理、分配与审计模型。

### 3.1 下一阶段：知识资产

| 任务 | 说明 | 交付物 |
| --- | --- | --- |
| 知识资产模型 | 定义文档、分块、版本、来源类型与租户隔离；与 `asset` 模块权限模型对齐 | 数据迁移、实体 / API 草案 |
| 文档接入 | 支持上传或登记 Markdown / Word / PDF 等常见格式，完成解析、分块与元数据提取 | 上传接口、解析 Worker 或同步解析链路 |
| 检索与引用 | 在智能体节点按流程上下文检索片段，限制引用范围并写入审计 | 检索 API、运行态引用留痕 |
| 权限与分配 | 复用租户能力池、资源范围与分配卡片，控制谁可管理 / 检索哪些知识 | 租户管理分配入口、能力资产页签 |
| 文档样例引入 | 将仓库 `docs/` 产品文档作为首批知识资产导入，验证检索质量与引用边界 | 导入脚本、演示数据与验收记录 |
| 与能力资产勾稽 | 智能体模板可声明可用知识集合；流程发布校验引用版本 | 设计态配置、发布校验规则 |

设计约束：

- 知识资产是阶段二新增资产类型，不回到阶段一已移除的「泛化知识库面板」心智。
- 检索结果必须可追溯：记录命中文档、版本、分块 ID 和调用上下文。
- 敏感文档默认受租户与资源范围约束，不在未授权智能体节点中可见。

### 3.1.1 附件输入与识别（知识资产前置能力）

本轮已完成第一版端到端代码实现，产品决策与落地范围如下：

- 系统管理新增“系统配置”页签，首项为附件识别配置；MinerU 密钥加密保存，健康连接测试已实现，逐格式真实解析测试列入稳定化范围。
- 输入节点新增“附件”字段，协议使用 `fieldType=file`；字段可配置扩展名子集、文件数、单文件大小、是否识别和识别失败是否阻断，具体识别引擎继承系统配置。
...
- 工作流变量保存不可变附件引用集合，智能体按权限和上下文预算读取解析正文，不把附件正文当作系统指令。
- 定时任务第一版不支持包含任何附件字段的流程，无论字段必填或选填都禁止创建或更新定时任务，避免临时上传 ID 被跨运行复用。

建议按以下垂直切片实施：

| 切片 | 交付范围 | 关键验证 |
| --- | --- | --- |
| A. 契约与治理（已实现） | 系统附件配置、数据表、OpenAPI、字段配置、扩展名 / 文件签名 / 大小校验、MinIO 原文件存储 | 系统管理员权限、密钥不回显、跨租户与伪造附件 ID 拒绝 |
| B. 解析链路（已实现第一版） | Java 解析器、MinerU 适配器、RabbitMQ 命令、幂等状态机、解析结果存储 | 已补本地文本 / DOCX / XLSX 测试；目标 MinerU 部署仍需逐格式联调 |
| C. 设计态与运行态（已实现） | 输入字段配置抽屉、上传 / 删除 / 进度 UI、待办提交与变量快照、智能体内容注入、分类预览 | 发布校验、必填、多文件、识别阻断与刷新恢复 |
| D. 稳定化（部分实现） | 平台 / 字段配额、保留期清理、操作审计、下载权限、文件签名与危险扩展名拦截 | 压缩炸弹深度检测、真实 MinerU 解析测试和并发压测仍需补充 |

上线前仍需用目标部署环境的 MinerU 版本确认请求字段及同步 / 异步响应格式。复杂识别的扩展名由系统管理员显式配置，因此不同端点对 `doc/xls` 的支持差异通过“配置白名单 + 对应格式样例测试 + 明确失败”处理，不做隐式本地兜底。

### 3.2 立即优先级（并行）

| 任务 | 说明 | 交付物 |
| --- | --- | --- |
| 阶段一端到端验收 | 固化登录选租户、能力分配、流程发布、发起运行、智能体 / MCP、人工审核、Word / Excel / 邮件 / Webhook 交付和审计追溯的标准验收脚本 | 验收脚本、测试数据和验证记录 |
...
| 知识资产 | 能力资产（阶段二扩展） | 企业文档、分块、版本与检索结果；与租户分配和智能体引用勾稽 |
...
| 2026-07-06 | `./gradlew test` | 通过：Excel 工作簿交付渲染、运行态路由、系统管理配置、Word/文件交付回归全量后端测试 |
| 2026-07-06 | `pnpm lint:web` / `pnpm build:web` | 通过：流程设计 Excel 多 Sheet 配置和系统管理 Excel 交付类型前端复验；Vite 仍提示 Ant Design vendor chunk 超过 500 kB |
...
| 2026-07-20 | `pnpm --filter @agentum/web lint` / `pnpm --filter @agentum/web build` | 通过：流程导入无效模型清空、单智能体完成提交与前端必选校验复验；Vite 仍提示部分 chunk 超过 500 kB |
| 2026-07-20 | `./gradlew :apps:api:test --no-daemon` | 通过：模型引用发布校验与既有后端全量回归 |
| 2026-07-23 | `./gradlew test --no-daemon` | 通过：系统 / 租户日志上下文、认证请求、节点执行、附件解析及既有后端全量回归 |
| 2026-07-23 | `docker compose config` / `local,logfile` 启动检查 / `git diff --check` | 通过：Compose 默认启用 `prod,logfile` 并挂载日志目录，文件 Appender 可正常初始化且配置空白检查通过 |
...
| 2026-09-14 | `./gradlew test` / `pnpm lint:web` / `pnpm build:web` / OpenAPI YAML 与 Schema 引用检查 / `git diff --check` | 通过：流程版本折叠、角色/部门/人员读取授权、最终可见人员解析及既有前后端回归；Vite 仍提示部分 chunk 超过 500 kB |
