...
- 流程输入字段支持文本、日期和下拉类型，字段配置统一使用右侧独立抽屉，并复用智能体配置的“左侧分类菜单 + 右侧当前配置”布局；文本和下拉的预设值默认允许业务人员修改，日期字段可按年、年月、年月日三级手工选择，当前年、当前日期、当前年月和上个年月也统一收敛到日期字段配置。人工发起时按配置自动预填并控制是否可修改，定时任务在每次执行时按计划触发时间和统一业务时区重新计算，解析结果进入输入变量快照。
