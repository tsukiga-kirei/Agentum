...
- 流程设计页签调整为“总览 / 全部流程 / 我的流程”；发布校验收敛到流程详情抽屉动作，避免和流程列表重复。
- 流程设计页按授信报告闭环重做交互心智：输入公司全称后，智能体结合 Skill 与 MCP 获取数据，智能体集群支持多智能体并行和拼接规则，用户最终审查并交付；事实类数据只能重新获取，模型文本可重新生成或追问修改。
...
- 流程设计页已重塑为共享协作流程库：总览改为可跳转功能卡，全部流程展示当前租户可见流程，我的流程通过 `scope=mine` 只筛当前创建人；流程卡点击先打开右侧详情抽屉，再进入积木设计或执行发布校验。
