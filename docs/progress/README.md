...
- 系统管理页已改为左侧模块菜单 + 右侧工作台：平台概览统计、租户状态、模型供应商与全局能力注册、租户能力授权；数据来自 `/api/system/*`（需 system_admin 登录）。
...
- 新增系统管理 API（`/api/system`）：概览统计、租户列表与状态更新、模型供应商与全局能力注册、租户能力授权；仅 `system_admin` 可访问。
- 新增 `TimeConfiguration` 提供可注入的 `Clock`，便于服务层时间与测试。
- 新增 Flyway `V202605100001` 演示种子：模型供应商、系统能力、租户授权示例数据。
...
| 前端系统管理页 | 从静态卡片改造成平台概览、租户管理、能力管理、模型管理、租户授权、系统审计等顶层菜单 | 已接 `/api/system/*`：左侧模块菜单 + 表格与注册弹窗 |
...
| 后端 package 边界 | 建立认证、租户、组织、权限、系统管理包结构 | 已推进认证、租户、组织、权限；已新增 `system` 包（模型供应商、全局能力、租户授权、租户状态） |
