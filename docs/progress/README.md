...
- Word 标题与手动空行样式补齐（2026-07-20）：保留首行单独居中的原有设置，并把一至五级标题对齐并入对应级别的标题配置；全局段落区只保留 Word 段距，个性化规则可在目标段落前后插入等价于手动回车的空段落，并按规则选择继承目标段落、继承正文或固定磅值，空段落自身段前段后归零以避免间距重复叠加。
