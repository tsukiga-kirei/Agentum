...
Skill 与 MCP 的工具发现、参数、调用、失败恢复和脚本执行边界详见 [Skill 与 MCP 运行机制](./skill-mcp-runtime-guide.md)。
...
1. **节点发布快照里的 `config_snapshot`**
   ...
   - `maxAgentIterationsPerTurn`（每次初次执行或追问的最大推理次数；必须由单智能体或每个集群子智能体显式配置）
...
每个正常循环都会重新发送当前节点全部可用工具定义；工具执行结果则追加为后续轮次的 `tool` message。达到上限后的兜底汇总调用只携带既有消息，不再发送 tools。追问会把本节点历史问答写入 `conversationHistory` 后重新进入相同循环，因此追问轮次同样会重新解析并发送当前节点的 MCP / Skill / `final_answer`。

循环次数不是运行时代填的固定值。设计器从后端目录读取建议值与平台最大值，每个节点显式保存 `maxAgentIterationsPerTurn`；首次执行和每次追问都会重新计数。平台上限通过 `AGENTUM_AGENT_MAX_ITERATIONS_PER_TURN` 调整，建议值通过 `AGENTUM_AGENT_SUGGESTED_ITERATIONS_PER_TURN` 调整，建议值只用于设计态，不作为旧快照的运行时兜底。
...
### 4.7 本地 AI 请求/响应 DEBUG 链路

`local` profile 默认开启 `OpenAiCompatibleModelChatClient` DEBUG 日志。每次物理模型调用会输出成对的：

- `AI调用链路-请求`：实际发送的 messages、tools、模型参数；
- `AI调用链路-响应`：正文、`final_answer` / MCP 等工具调用参数、Token 用量和结束原因。

两条日志使用 `runId`、`nodeRunId`、`modelCallLogId` 串联，其中 `modelCallLogId` 对应 `model_call_logs.id`。API Key、Token、Authorization、Cookie 等凭证会先脱敏，长文本会限制长度。可通过 `AGENTUM_AI_DEBUG_LOG_LEVEL=INFO` 关闭；生产 profile 默认不启用这套正文级 DEBUG 日志。
...
- 运行页工具步骤会显示“已读取文件”的摘要，并允许展开查看本次实际回写模型的 Skill 内容；SSE 事件详情最多保留 4000 字，完整 Agent observation 仍受 Skill 单次读取上限约束。

当前 Skill 只读取文本，不会执行 `SKILL.md` 或 `skill.yaml` 引用的 Shell、Python、Node.js 脚本。需要真实动作时应使用 MCP；完整边界见 [Skill 与 MCP 运行机制](./skill-mcp-runtime-guide.md)。
...
| deliveryMode | 行为 |
| --- | --- |
| `capability` | 解析绑定的交付能力，按通道执行 |

已落地通道：

- 系统内置邮箱：`EmailDeliveryService`
- Webhook：HTTP POST
- 系统内置 Word / Excel 文件：生成 docx 或 xlsx 并写入 MinIO/S3 兼容对象存储
- 自定义交付能力：读取系统登记配置
...
- 智能体集群支持 `collaborative` 协同处理、`relay` 接力处理与 `intent` 意图分派；历史 `parallel` / `sequential` 数据由迁移脚本清洗为新枚举，运行态不再保留旧值兼容分支。意图分派先运行受控分类器，只接受设计态 `intentRoutes` 白名单里的 `intentCode`，再按意图清单顺序只执行命中的子智能体；集群最终输出变量默认 `cluster_result`，输出模板只组合子智能体输出，未命中可中止、转交指定智能体或返回固定话术，路由结果写入节点输出与进度事件。
