...
当前处于**阶段一：框架与基础治理**。身份、租户、组织权限、能力资产、流程设计草稿和业务工作台运行态已打通；后端运行态已能按租户模型分配调用 OpenAI 兼容模型、按 MCP SSE `tools/call` 调用能力并生成交付记录。异步 Worker、复杂文档生成、高风险审批、重试补偿和完整运行审计页仍在建设中。详见 [当前进度](./docs/progress/README.md)。
