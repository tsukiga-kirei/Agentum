...
当前处于**阶段一：框架与基础治理**。身份、租户、组织权限、能力资产与流程设计草稿已打通；运行态执行、MCP 网关与交付闭环仍在建设中。详见 [当前进度](./docs/progress/README.md)。
...
- **智能体节点运行时**（建设中） — 节点内调用模型与工具，支持并行子任务、追问确认与输出校验
- **人工卡点与交付**（建设中） — 输入、审核、暂停恢复，交付至 Word / PDF、邮件、OA、Webhook 等
- **运行审计**（建设中） — 追溯运行链路、变量快照、工具调用与交付记录
...
packages/ui               前端通用 UI
capabilities/             产品运行时能力源码（Skill、MCP、提示词、交付适配器）
